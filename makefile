...
SRCS = firmware.c display.c button.c raytracer.c main.c 
INCLUDES += ./
...
all: build/mdoom.bin
	hexdump -v -e '"\\" "x" 1/1 "%02X"' build/mdoom.bin
	wc -c build/mdoom.bin
...
build/mdoom.bin: build/mdoom.elf build/mdoom.lst
	$(OCPY) $< $@ -O binary
	$(SZ) $<

build/mdoom.lst: build/mdoom.elf build
	$(ODUMP) -D $< > $@

build/mdoom.elf: $(OBJS)
	@echo "Linking $@"
	$(LD) $^ $(LDFLAGS) -o $@
